...
1. Ground yourself in current knowledge (1–3 searches; read enough to reason, not
   exhaustively; prefer primary/authoritative sources). **If the invocation prompt enables
   Perplexity this run** (GROUNDING: Perplexity), use `perplexity_search` for a broad scan
   that returns ranked, cited links, then `WebFetch` the few most relevant; use
   `perplexity_research` for a deeper multi-source pass on unfamiliar areas. **If it says to
   use built-in search only**, use `WebSearch`/`WebFetch` and do not call perplexity_* tools.
