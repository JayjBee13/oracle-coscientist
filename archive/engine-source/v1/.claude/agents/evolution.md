...
- **Grounding:** fix a weakness by pulling in specific evidence. If the invocation prompt
  enables Perplexity this run, use `perplexity_search` (cited broad scan) or
  `perplexity_research` (deep), then `WebFetch` the key links; otherwise use built-in
